...
#CodeForm class 
class CodeForm(Form):
    language= StringField('Language', [validators.Length(min=1, max=20)])
    body = TextAreaField('Body', [validators.Length(min=1)])
    input_val = TextAreaField('input_val')

#Write Code
@app.route("/write_code", methods=['POST', 'GET'])
@is_logged_in
def write_code():
    form1 = CodeForm(request.form)
    
    lang= form1.language.data
    print(lang)
    body = form1.body.data
    input_val= form1.input_val.data
    ...
    print(r)
    return render_template('output_window.html', r=r)  #to be completed
